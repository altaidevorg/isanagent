...
**Plugins & Harness Packs** (`src/plugins/`): extensible bundles discovered hierarchically from `<workspace>/.agents/plugins/` and `~/.isanagent/plugins/`. A pack bundle contains `plugin.json` (or `pack.toml`), declarative subagents (`agents/<name>/AGENT.md` with YAML frontmatter), skills (`skills/<name>/SKILL.md`), rules (`rules/*.md`), lifecycle hooks (`hooks.json`), and MCP servers (`mcp_config.json`). The CLI provides `isanagent pack install <repo_url>`, `isanagent pack list`, and `isanagent pack remove <name>`. At startup, `PluginRegistry` ingests skills into `SkillRegistry`, subagents into `AgentRegistry`, and injects overlay prompts into the coordinator system prompt.
