...
pub enum BusMessage {
    ...
    /// Interrupt an active reasoning loop for a chat; clears FIFO queued inbounds for that chat.
    Cancel(String),
    /// Interrupt ONE exact active reasoning run (preferred by hosts with a lifecycle coordinator).
    CancelRun { chat_id: String, run_id: String },
    /// Apply new user direction to one exact active run at its next safe boundary.
    Steer { chat_id: String, run_id: String, content: String },
    /// /background: promote the in-flight synchronous tool call to an ExecutionJobManager job.
    PromoteSyncToBackground(String),
    /// TUI /new + resume: retarget terminal-scoped thought/progress telemetry.
    SetTerminalSessionChat { chat_id: String },
    /// /model slash command: atomic runtime provider/model switch.
    SwitchModel { provider_name: String, model_name: String, base_url: String, api_key: String },
    /// Caller-driven compaction request (session_key via clarification_session_key()).
    TriggerCompaction { session_key: String, focus_instructions: Option<String>, trigger: Option<CompactionTrigger> },
    /// TUI /skills add: remote skill installation.
    InstallSkill { repo_url: String, skill_name: Option<String> },
    /// Incremental provider streaming chunk for real-time client rendering.
    StreamDelta { chat_id: String, chunk: crate::traits::StreamChunk },
...
2. `sandbox_dir` (Inner Rim): This is the designated execution field, defaults to `workspace_dir/workspace` (override via the `sandbox` config value). This is where the Agent expects to load `AGENTS.md` and read `skills/`.
...
**Builtin tools** (`[harness.builtin_tools] enabled = true` by default): filesystem, web, `exec`, cron, memory, todos, `load_skill_instructions`, `search_tools`, and other core host tools. Set **`enabled = false`** to leave only plugin MCP tools (plus any independently enabled harness tools such as subagents). Tool JSON schemas are sent only through the provider `tools` array — never dumped into the system prompt.

**Default identity prompt** (`[harness.default_prompt] enabled = true` by default): merges `AGENTS.md`, `SOUL.md`, `USER.md`, and `MEMORY.md`. Set **`enabled = false`** so plugin overlay prompts (and always-on skill bodies) are the specialization surface.
...
**Agent Plugins 1.0** (`src/plugins/`): open standard package format conforming to Agent Plugins 1.0 (`agent-plugins.org`). Plugins are discovered hierarchically from `~/.agent-plugins/`; `<global_root>/plugins/` and `<global_root>/packs/` (global root is `resolve_workspace_root(None)`, default `~/.isanagent/`); `<workspace>/.agents/plugins/`, `<workspace>/.agents/packs/`, `<workspace>/.isanagent/plugins/`, `<workspace>/.isanagent/packs/`, `<workspace>/plugins/`, and `<workspace>/assets/plugins/`. Workspace scans run last and win plugin-name collisions. A plugin contains `plugin.json` (`$schema: "https://agent-plugins.org/schemas/1.0.0/plugin.schema.json"`), standard skills (`skills/<name>/SKILL.md`), standard MCP servers (`mcp.json`), and client extension namespace `dev.altai.isanagent/` containing declarative subagents (`agents/<name>/AGENT.md`), rules (`rules/*.md`), and hooks (`hooks.json`). The CLI provides `isanagent plugin install <repo_url>`, `isanagent plugin list`, and `isanagent plugin remove <name>`. At startup, `PluginRegistry` ingests skills into `SkillRegistry`, subagents into `AgentRegistry`, and injects extension overlay prompts into the coordinator system prompt.
...
Write a `SKILL.md` inside the sandbox skills directory: `workspace/skills/{skill-name}/SKILL.md`.
