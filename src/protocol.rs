//! Channel-agnostic outbound notice protocol (audit X10).
//!
//! Metadata keys and constructor functions for the synthetic `OutboundMessage`
//! "pseudo-outbounds" that core components emit (tool notices, thoughts, execution /
//! sub-agent / background-job strips, channel errors). Core (host, agent loop, execution
//! plane, tools) builds these; frontends (terminal, Ratatui UI, API) interpret the keys.
//! Both directions may depend on this module — neither may depend on the other.
...
/// Synthetic tool-notice marker (paired with [`ISANAGENT_TOOL_PHASE`]).
pub const ISANAGENT_TOOL_NOTIFY: &str = "isanagent_tool_notify";
/// Phase of a [`ISANAGENT_TOOL_NOTIFY`] notice (`call`, `result`, `fail`, `progress`).
pub const ISANAGENT_TOOL_PHASE: &str = "isanagent_tool_phase";
...
/// Tail-truncation used by both notice previews here and terminal rendering.
pub(crate) fn truncate_leading_ellipsis(s: &str, max_chars: usize) -> String {
    let n = s.chars().count();
    if n <= max_chars {
        return s.to_string();
    }
...
pub fn build_agent_thought_terminal_notice(
    ...
    let mut metadata = HashMap::new();
    metadata.insert(ISANAGENT_AGENT_THOUGHT.to_string(), json!(true));
    if let Some(id) = background_job_id.filter(|s| !s.is_empty()) {
        metadata.insert(
            crate::bus::METADATA_BACKGROUND_JOB_ID.to_string(),
            json!(id),
...
pub fn build_channel_error_notice(
    ...
    let mut metadata = HashMap::new();
    if channel == "terminal" {
        metadata.insert(ISANAGENT_TERMINAL_ERROR.to_string(), json!(true));
    }
    OutboundMessage {
        channel: channel.to_string(),
        chat_id: chat_id.to_string(),
