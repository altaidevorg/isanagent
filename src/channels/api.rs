...
mod tests {
    ...
    fn build_state(
        ...
        let memory_actor =
            SqliteMemoryActor::new(db_path.to_str().expect("utf8 db path"), None).expect("memory actor");
        let memory_node =
...
"#,
    ...
    async fn responses_route_still_works_when_ui_is_enabled() {
        ...
        let memory_actor =
            SqliteMemoryActor::new(temp.db_path().to_str().expect("utf8"), None).expect("memory actor");
        let memory_node =
            NodeHandle::<MemoryMessage>::new(memory_actor, 100, 1, Duration::from_millis(5));
        let state = ApiState {
            bus_tx,
