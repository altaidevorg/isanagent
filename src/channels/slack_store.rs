...
mod tests {
    ...
    #[tokio::test]
    async fn slack_user_profile_store_round_trips_profiles() {
        let db_path = temp_db_path("slack-user-profile-roundtrip");
        let store = SlackUserProfileStore::new(&db_path).unwrap();
        ...
        let stored = store.get("U123").await.unwrap().unwrap();
    ...
    #[tokio::test]
    async fn slack_user_profile_store_updates_existing_profiles() {
        let db_path = temp_db_path("slack-user-profile-update");
        let store = SlackUserProfileStore::new(&db_path).unwrap();
        ...
        store
            .upsert(
                "U123",
                &StoredSlackUserProfile {
                    display_name: "Jane Updated".into(),
                    fetched_at_unix_secs: 456,
                },
            )
            .await
            .unwrap();
        ...
        assert_eq!(stored.display_name, "Jane Updated");
        assert_eq!(stored.fetched_at_unix_secs, 456);
