...
| `env_scrub_secrets` | **Local provider only.** When **true**, strip secret-bearing environment variables from each child process — any name ending in `_API_KEY` / `_TOKEN` / `_SECRET` / `_ACCESS_KEY` / `_PASSWORD` (covers `OPENAI_API_KEY`, `HF_TOKEN`, `JUPYTER_TOKEN`, `AWS_SECRET_ACCESS_KEY`, `SSH_PASSWORD`, …) plus `env_extra_secret_vars`. Default **false** (full host env is forwarded). ⚠️ Enabling this also removes credentials that *legitimate* code may need (e.g. `HF_TOKEN` to pull gated models); turn it on for **untrusted/prompt-injected** workloads where preventing the agent's own credentials from being `printenv`'d/exfiltrated matters more. `PATH`/`HOME`/locale and ML vars (`CUDA_*`, `LD_LIBRARY_PATH`, `HF_HOME`, …) are always preserved. |
