...
| `env_scrub_secrets` | When **true**, strip secret-bearing environment variables from every child the agent spawns — the local execution provider **and** the `shell_exec` / `python_run` builtin tools. Matches names ending in `_API_KEY` / `_APIKEY` / `_TOKEN` / `_SECRET` / `_SECRET_KEY` / `_ACCESS_KEY` / `_KEY_ID` / `_PASSWORD` / `_PASSWD` (covers `OPENAI_API_KEY`, `HF_TOKEN`, `JUPYTER_TOKEN`, `AWS_SECRET_ACCESS_KEY`, `AWS_ACCESS_KEY_ID`, `SSH_PASSWORD`, …) plus `env_extra_secret_vars`. Default **false** (full host env is forwarded). ⚠️ Enabling this also removes credentials that *legitimate* code may need (e.g. `HF_TOKEN` to pull gated models); turn it on for **untrusted/prompt-injected** workloads. `PATH`/`HOME`/locale and ML vars (`CUDA_*`, `LD_LIBRARY_PATH`, `HF_HOME`, `TOKENIZERS_PARALLELISM`, …) are always preserved. **Limitations:** this is a name-based denylist — it does **not** catch connection strings with embedded creds (`DATABASE_URL`, `*_DSN`), file-path credentials (`GOOGLE_APPLICATION_CREDENTIALS`, `~/.aws/credentials`, `~/.netrc`), or bare un-prefixed names; add those to `env_extra_secret_vars`. It stops **env-var** leakage only — it does not sandbox the filesystem or block network egress. |
| `env_extra_secret_vars` | Extra exact env var names (case-insensitive) to strip when `env_scrub_secrets` is on, e.g. `["MY_INTERNAL_CRED"]`. |
