...
| `rlimit_address_space_bytes` | **Local provider only.** `RLIMIT_AS` — max virtual address space (bytes) for each run's child, applied via `setrlimit` in the post-fork hook. Default **unset** (`0` = unset). Guards against memory-exhaustion loops. **Linux-enforced; macOS best-effort** (many runtimes reserve huge virtual space — prefer this on Linux deployments). Best-effort: a failed `setrlimit` is not fatal. |
| `rlimit_cpu_secs` | **Local provider only.** `RLIMIT_CPU` — max CPU seconds per run (child gets `SIGXCPU`). Default **unset**. Guards against runaway-CPU loops. |
| `rlimit_max_processes` | **Local provider only.** `RLIMIT_NPROC` — fork-bomb guard. Default **unset**. ⚠️ **Per-UID, not per-process:** the limit counts **all** of the agent user's existing processes, so (a) a value not comfortably above the current baseline can be hit by *unrelated* processes, and (b) setting it too low can make the user's **other** processes fail `fork()` (`EAGAIN`). Choose a value above the UID's normal process count. |
| `rlimit_max_open_files` | **Local provider only.** `RLIMIT_NOFILE` — max open file descriptors for the child. Default **unset**. |
| `rlimit_max_file_size_bytes` | **Local provider only.** `RLIMIT_FSIZE` — max bytes of any file the child writes (child gets `SIGXFSZ`). Default **unset**. Guards against disk-fill loops. |
